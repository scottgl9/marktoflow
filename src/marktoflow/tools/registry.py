"""
Tool Registry for marktoflow framework.
...
class ToolRegistry:
    ...
    def _create_tool(
        ...
        if implementation.type == ToolType.MCP:
            from marktoflow.tools.mcp_bridge import MCPTool

            return MCPTool(definition, implementation)
        elif implementation.type == ToolType.OPENAPI:
            from marktoflow.tools.openapi import OpenAPITool

            return OpenAPITool(definition, implementation)
        else:
            from marktoflow.tools.custom import CustomTool
